...
# Очистка полей ввода
def clear():
    l_text_input.delete(0, ttk.END) # tk
    l_key_input.delete(0, tk.END)
    l_output.delete(0, tk.END)

# Запуск функций шифрования
def start():
    ...
    if method == 'Цезаря':
        ...
        if action == 'Зашифровать':
            outpup = str(f.encode_vijn(message, key, alp, b)) # output
            l_output.delete(0, tk.END)
            l_output.insert(0, output)
        elif action == 'Расшифровать':
        ...
        if action == 'Зашифровать':
            ...
            l_output.delete(0, ttk.END) #tk
            l_output.insert(0, output)

# Настройки графических элементов приложения
...
l_start = ttk.Button(frame_input_output, text="Старт", command=clear) # start
l_clear = ttk.Button(frame_input_output, text="clear", command=start) # clear
